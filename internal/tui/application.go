package tui

import (
	"io"
	"time"

	"charm.land/bubbles/v2/key"
	tea "charm.land/bubbletea/v2"
	"charm.land/lipgloss/v2"
	"codeberg.org/clambin/bubbles/helper"
	"github.com/clambin/xcoder/internal/pipeline"
)

const refreshInterval = 250 * time.Millisecond

type paneID string

const (
	queuePane paneID = "queue"
	logPane   paneID = "log"
)

var _ tea.Model = Application{}

// Application is the controller for the UI.
type Application struct {
	help         map[paneID]helper.Helper
	activePane   paneID
	configViewer string
	logViewer    logViewer
	keyMap       KeyMap
	queueViewer  queueViewer
	statusLine   statusLine
	width        int
	height       int
}

func New(queue Queue, config pipeline.Configuration) Application {
	styles := DefaultStyles()
	keyMap := DefaultKeyMap()

	return Application{
		configViewer: newConfigView(config, styles.Config),
		statusLine:   newStatusLine(queue, styles.Status),
		activePane:   queuePane,
		queueViewer:  newQueueViewer(queue, styles.QueueViewer, keyMap.QueueViewer),
		logViewer:    newLogViewer(keyMap.LogViewer, styles.LogViewer),
		keyMap:       keyMap,
		help: map[paneID]helper.Helper{
			queuePane: helper.New().Styles(styles.Help).Sections([]helper.Section{
				{Title: "General", Keys: []key.Binding{keyMap.Quit, keyMap.ShowLogs}},
				{Title: "Navigation", Keys: keyMap.QueueViewer.FilterTableKeyMap.ShortHelp()},
				{Title: "Media", Keys: []key.Binding{keyMap.QueueViewer.ActivateQueue, keyMap.QueueViewer.Convert, keyMap.QueueViewer.ShowFullPath}},
				{Title: "Filter", Keys: keyMap.QueueViewer.MediaFilterKeyMap.ShortHelp()},
			}),
			logPane: helper.New().Styles(styles.Help).Sections([]helper.Section{
				{Title: "General", Keys: []key.Binding{keyMap.Quit}},
				{Title: "Logs", Keys: keyMap.LogViewer.ShortHelp()},
			}),
		},
	}
}

func (a Application) LogWriter() io.Writer {
	return a.logViewer.LogWriter()
}

func (a Application) Init() tea.Cmd {
	return tea.Batch(
		a.statusLine.Init(),
		a.queueViewer.Init(),
		a.logViewer.Init(),
		func() tea.Msg { return autoRefreshUIMsg{} },
	)
}

func (a Application) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	//fmt.Printf("%#+v\n", msg)
	var cmd tea.Cmd
	switch msg := msg.(type) {
	case tea.WindowSizeMsg:
		return a.resizeComponents(msg)
	case autoRefreshUIMsg:
		return a, tea.Batch(
			func() tea.Msg { return refreshUIMsg{} },
			tea.Tick(refreshInterval, func(t time.Time) tea.Msg { return autoRefreshUIMsg{} }),
		)
	case logViewerClosedMsg:
		a.activePane = queuePane
		return a, nil
	case tea.KeyPressMsg:
		switch {
		case key.Matches(msg, a.keyMap.Quit) && !a.queueViewer.textFilterOn:
			// hard exit: no need to process any more messages
			return a, tea.Quit
		case key.Matches(msg, a.keyMap.ShowLogs) && !a.queueViewer.textFilterOn:
			a.activePane = logPane
			return a, nil
		default:
			switch a.activePane {
			case queuePane:
				a.queueViewer, cmd = a.queueViewer.Update(msg)
			case logPane:
				a.logViewer, cmd = a.logViewer.Update(msg)
			}
			return a, cmd
		}
	default:
		cmds := make([]tea.Cmd, 3)
		a.logViewer, cmds[0] = a.logViewer.Update(msg)
		a.queueViewer, cmds[1] = a.queueViewer.Update(msg)
		a.statusLine, cmds[2] = a.statusLine.Update(msg)
		return a, tea.Batch(cmds...)
	}
}

func (a Application) View() tea.View {
	components := []string{
		a.viewHeader(),
		"",
		a.statusLine.View(),
	}
	switch a.activePane {
	case queuePane:
		components[1] = a.queueViewer.View()
	case logPane:
		components[1] = a.logViewer.View()
	}
	v := tea.NewView(lipgloss.JoinVertical(lipgloss.Top, components...))
	v.AltScreen = true
	return v
}

func (a Application) resizeComponents(msg tea.WindowSizeMsg) (tea.Model, tea.Cmd) {
	a.width = msg.Width
	a.height = msg.Height
	a.statusLine = a.statusLine.SetSize(msg.Width, 1)
	// how much room is left for the panes?
	paneHeight := a.height - lipgloss.Height(a.viewHeader()) - 1
	// update the pane sizes
	a.queueViewer = a.queueViewer.SetSize(msg.Width, paneHeight)
	a.logViewer = a.logViewer.SetSize(msg.Width, paneHeight)

	a.help[logPane] = a.help[logPane].Width(a.width - lipgloss.Width(a.configViewer))
	a.help[queuePane] = a.help[queuePane].Width(a.width - lipgloss.Width(a.configViewer))
	return a, func() tea.Msg { return refreshUIMsg{} }
}

func (a Application) viewHeader() string {
	return lipgloss.JoinHorizontal(lipgloss.Left,
		lipgloss.NewStyle().Padding(1, 5, 0, 0).Render(a.configViewer),
		a.help[queuePane].View(),
	)
}
